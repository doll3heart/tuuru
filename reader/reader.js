...
const authorPhoneRenderDocument = document.documentElement.hasAttribute('data-author-phone-render')
var authorPhoneRenderCustom = null
...
function readerPhoneImageCssUrl(value) {
  // Escape CSS string syntax here; the complete style attribute is HTML-escaped
  // once by its caller. HTML-escaping the URL here corrupts signed query strings.
...
function getReaderLibraryState() {
  if (authorPhoneRenderDocument) return {books:[], identities:[], collections:[]}
  return readReaderLibrary(localStorage)
}

function commitReaderLibraryState(library) {
  if (authorPhoneRenderDocument) return false
...
function renderHome() {
  ...
  h += '</div>'
  // Tab panels
  h += '<div class="rd-panel" id="tabPersonal" role="tabpanel" aria-labelledby="rdTabPersonal"' + (activeTab === 'personal' ? '' : ' style="display:none" hidden') + '>' + (activeTab === 'personal' ? renderPersonalPage() : '') + '</div>'
  h += '<div class="rd-panel"' + (activeTab === 'library' ? '' : ' style="display:none"') + ' id="tabLibrary" role="tabpanel" aria-labelledby="rdTabLibrary"' + (activeTab === 'library' ? '' : ' hidden') + '>' + (activeTab === 'library' ? renderBookshelfPage() : '') + '</div>'
  h += '<div class="rd-panel"' + (activeTab === 'custom' ? '' : ' style="display:none"') + ' id="tabCustom" role="tabpanel" aria-labelledby="rdTabCustom"' + (activeTab === 'custom' ? '' : ' hidden') + '></div>'
  ...
  h += '</div>'
  render('app', h)
  if (activeTab === 'personal') bindPersonalPage(document)
  if (activeTab === 'library') bindBookshelfPage(document)
  if (activeTab === 'custom') renderCustomPage()
...
function readerDataPanelMarkup() {
  return '<section id="rdReaderDataPanel" class="rd-reader-data-panel" data-reader-data-panel aria-labelledby="rdReaderDataTitle"' +
    ...
    '<div class="rd-reader-data-intro"><div><h3 id="rdReaderDataTitle">恢复阅读数据</h3>' +
    '<p>从旧备份恢复阅读进度、身份组、占位符、书签和便携外观设置。</p></div>' +
...
function bindReaderDataPanel(root) {
  ...
  toggle.onclick = function() {
    ...
    if (_readerDataPanelOpen) {
      var firstAction = panel.querySelector('[data-reader-data-import]')
  ...
  continueButton.onclick = function() {
    ...
    if (!result.ok) {
      ...
      status.textContent = result.rollbackOk
        ...
        : '空间仍然不足，且部分正文缓存未能恢复；请保留原作品文件，再检查本地存储空间。'
      return
    }
    closeReaderImportDialog({restoreFocus:false})
...
function readerAppearancePackageTransferMarkup() {
  return '<section class="reader-appearance-transfer" aria-labelledby="readerAppearanceTransferTitle">' +
    '<div class="rs-group-heading"><span id="readerAppearanceTransferTitle">导入美化包</span><small>恢复文章、手机与 App 外观</small></div>' +
    '<p>导入已有美化包只更新外观设置，不会替换昵称 / ID、头像、简介、作品、书架、密码或阅读记录。</p>' +
    '<p class="reader-appearance-transfer-assets">可恢复包内的壁纸、个人主页顶部图、字体、图标和桌面组件外观；组件便签只留在本机。</p>' +
...
function buildPhoneHTML(pd, custom, watermark, flowStep) {
  ...
  if (skin.wallpaperType === 'image' && skin.wallpaperImage) {
    readerBgStyle += ';background-image:' + readerPhoneImageCssUrl(skin.wallpaperImage) + ';background-size:cover;background-position:center'
  }
...
function openReaderApp(type, contactIndex, connectionConfirmed, flowStep, navigationContext) {
  ...
  var activeSlot = !authorPhoneRenderDocument && _work && _work.id ? readerActiveSlot(savedReaderBook(_work.id)) : null
...
function getPhoneCustom() {
  if (authorPhoneRenderDocument) return normalizePhoneCustom(authorPhoneRenderCustom)
...
function renderPhonePreview(ct, options) {
  ...
  if (ct.wallpaperType === 'image' && ct.wallpaperImage) {
    frameBgStyle += ';background-image:' + readerPhoneImageCssUrl(ct.wallpaperImage) + ';background-size:cover;background-position:center'
  }
  ...
  if (fontUpload) fontUpload.onclick = function() {
    ...
    inp.onchange = function() {
      ...
      r.onload = function() {
        ...
        openReaderCustomizePanelLegacy()
      }
      r.readAsDataURL(file)
    }
    inp.click()
...
// Phone and profile appearance editor UI lives in reader-appearance-workbench.js.

function phoneDesktopWidgetDefinition(productId) {
  return PHONE_DESKTOP_WIDGET_PRODUCTS.find(function(item) { return item.id === productId }) || PHONE_DESKTOP_WIDGET_PRODUCTS[0]
}
...
// Per-app appearance editor UI lives in reader-appearance-workbench.js.
...
let readerAppearanceWorkbenchPromise = null
let readerAppearanceActivationPromise = null

function loadReaderAppearanceWorkbench() {
  if (!readerAppearanceWorkbenchPromise) {
    readerAppearanceWorkbenchPromise = import('./reader-appearance-workbench.js').catch(function(error) {
      readerAppearanceWorkbenchPromise = null
      throw error
    })
  }
  return readerAppearanceWorkbenchPromise
}

function readerAppearanceRuntimeAdapter() {
  return {
    PHONE_CUSTOM_DECORATION_MAX_ITEMS:PHONE_CUSTOM_DECORATION_MAX_ITEMS,
    PHONE_CUSTOM_DECORATION_SIZES:PHONE_CUSTOM_DECORATION_SIZES,
    PHONE_DESKTOP_WIDGET_FIELDS:PHONE_DESKTOP_WIDGET_FIELDS,
    PHONE_DESKTOP_WIDGET_PRODUCTS:PHONE_DESKTOP_WIDGET_PRODUCTS,
    PHONE_HOME_CELL_HEIGHT:PHONE_HOME_CELL_HEIGHT,
    PHONE_HOME_CELL_WIDTH:PHONE_HOME_CELL_WIDTH,
    PHONE_HOME_COLUMNS:PHONE_HOME_COLUMNS,
    PHONE_HOME_MAX_PAGES:PHONE_HOME_MAX_PAGES,
    PHONE_HOME_ROWS:PHONE_HOME_ROWS,
    READER_CALL_BACKGROUND_DEFAULT:READER_CALL_BACKGROUND_DEFAULT,
    READER_CALL_BACKGROUND_PRESETS:READER_CALL_BACKGROUND_PRESETS,
    READER_CUSTOM_CSS_MAX_LENGTH:READER_CUSTOM_CSS_MAX_LENGTH,
    _work:_work,
    addReaderLocalFont:addReaderLocalFont,
    applyCompiledReaderStyle:applyCompiledReaderStyle,
    applyCustomFonts:applyCustomFonts,
    applyPhoneCustomCss:applyPhoneCustomCss,
    applyReaderAppCustomCss:applyReaderAppCustomCss,
    avatarColor:avatarColor,
    bindReaderAppearancePackageTransfer:bindReaderAppearancePackageTransfer,
    bindReaderAppearancePager:bindReaderAppearancePager,
    bindReaderAppearanceSectionStates:bindReaderAppearanceSectionStates,
    bindReaderAppearanceUndo:bindReaderAppearanceUndo,
    bindReaderPhoneHomePager:bindReaderPhoneHomePager,
    boundedReaderSetting:boundedReaderSetting,
    canonicalReaderCallBackgroundDataUrl:canonicalReaderCallBackgroundDataUrl,
    clearAppearanceDraft:clearAppearanceDraft,
    compileScopedReaderCss:compileScopedReaderCss,
    createReaderCustomDecorationId:createReaderCustomDecorationId,
    cuCollapsibleSubsection:cuCollapsibleSubsection,
    cuColorRow:cuColorRow,
    cuRow:cuRow,
    cuSettingsSection:cuSettingsSection,
    cuSettingsSectionEnd:cuSettingsSectionEnd,
    cuSettingsSectionStart:cuSettingsSectionStart,
    cuSettingsSubsection:cuSettingsSubsection,
    cuShapeBtn:cuShapeBtn,
    cuSliderRow:cuSliderRow,
    defaultReaderAppSettings:defaultReaderAppSettings,
    defaultReaderMessageSettings:defaultReaderMessageSettings,
    deleteReaderLocalFont:deleteReaderLocalFont,
    enhanceReaderAppearanceRanges:enhanceReaderAppearanceRanges,
    esc:esc,
    escapeHtmlAttribute:escapeHtmlAttribute,
    focusReaderAppearanceSection:focusReaderAppearanceSection,
    getAppSettings:getAppSettings,
    getPhoneCustom:getPhoneCustom,
    inspectReaderAppearanceImage:inspectReaderAppearanceImage,
    isSafeImageUrl:isSafeImageUrl,
    movePhoneHomeItem:movePhoneHomeItem,
    normalizeDynamicIslandStyle:normalizeDynamicIslandStyle,
    normalizePhoneCustom:normalizePhoneCustom,
    normalizePhoneDesktopWidgets:normalizePhoneDesktopWidgets,
    normalizePhoneHomeLayout:normalizePhoneHomeLayout,
    normalizedReaderBubbleFontWeight:normalizedReaderBubbleFontWeight,
    normalizedReaderBubbleSkin:normalizedReaderBubbleSkin,
    normalizedReaderCallBackgroundSettings:normalizedReaderCallBackgroundSettings,
    normalizedReaderChatAppearanceSettings:normalizedReaderChatAppearanceSettings,
    normalizedReaderGallerySettings:normalizedReaderGallerySettings,
    openCuModal:openCuModal,
    orderedForumPosts:orderedForumPosts,
    phoneCustomDecorationSizeForDimensions:phoneCustomDecorationSizeForDimensions,
    phoneHomeDefinitions:phoneHomeDefinitions,
    phoneHomeFootprint:phoneHomeFootprint,
    phoneHomeItemStyle:phoneHomeItemStyle,
    readAppearanceDraft:readAppearanceDraft,
    readReaderCallBackgroundFile:readReaderCallBackgroundFile,
    readReaderCustomDecorationFile:readReaderCustomDecorationFile,
    readerAppCssType:readerAppCssType,
    readerAppearancePackageTransferMarkup:readerAppearancePackageTransferMarkup,
    readerAppearancePagerMarkup:readerAppearancePagerMarkup,
    readerBubbleSkinClass:readerBubbleSkinClass,
    readerBubbleSkinVariables:readerBubbleSkinVariables,
    readerCallBackgroundPresentation:readerCallBackgroundPresentation,
    readerChatReadabilityVariables:readerChatReadabilityVariables,
    readerChatTonePresentation:readerChatTonePresentation,
    readerColorInputValue:readerColorInputValue,
    readerCustomDecorationName:readerCustomDecorationName,
    readerImageAttributes:readerImageAttributes,
    readerLocalFontFamily:readerLocalFontFamily,
    readerOwnDataRecord:readerOwnDataRecord,
    readerPhoneCustomDefaults:readerPhoneCustomDefaults,
    readerPhoneData:readerPhoneData,
    readerPhoneText:readerPhoneText,
    readerPhoneWidgetPreviewData:readerPhoneWidgetPreviewData,
    readerPlainRecord:readerPlainRecord,
    readerReadableTextColor:readerReadableTextColor,
    readerThreadDisplayName:readerThreadDisplayName,
    renameReaderLocalFont:renameReaderLocalFont,
    renderCustomPage:renderCustomPage,
    renderPhoneCustomDecoration:renderPhoneCustomDecoration,
    renderPhoneDesktopWidgets:renderPhoneDesktopWidgets,
    renderPhonePreview:renderPhonePreview,
    renderPhoneShoppingList:renderPhoneShoppingList,
    renderPhoneShoppingTabs:renderPhoneShoppingTabs,
    renderReaderAppearanceImageChoice:renderReaderAppearanceImageChoice,
    replaceReaderLocalFont:replaceReaderLocalFont,
    safePhoneCustomFontFamily:safePhoneCustomFontFamily,
    sanitizeCssColor:sanitizeCssColor,
    savePhoneCustom:savePhoneCustom,
    setPhoneHomePageCount:setPhoneHomePageCount,
    setReaderRangeOutput:setReaderRangeOutput,
    shouldShowPhoneTimestamp:shouldShowPhoneTimestamp,
    shouldUseMotion:shouldUseMotion,
    showReaderToast:showReaderToast,
    validatedReaderCallBackgroundCandidate:validatedReaderCallBackgroundCandidate,
    verifiedReaderCallBackgroundImages:verifiedReaderCallBackgroundImages,
    verifiedReaderImageLuminance:verifiedReaderImageLuminance,
    verifyReaderCallBackgroundDataUrl:verifyReaderCallBackgroundDataUrl,
    writeAppearanceDraft:writeAppearanceDraft,
  }
}

function activateReaderAppearanceWorkbench(activate) {
  if (readerAppearanceActivationPromise) return readerAppearanceActivationPromise
  var loaded = loadReaderAppearanceWorkbench().catch(function(error) {
    console.error('Reader appearance workbench failed to load', error)
    showReaderToast('外观编辑器加载失败，请重试', 'error')
    return null
  })
  var activation = loaded.then(function(workbench) {
    if (!workbench) return
    return activate(workbench, readerAppearanceRuntimeAdapter())
  }).catch(function(error) {
    console.error('Reader appearance workbench failed to open', error)
  })
  var guardedActivation = activation.finally(function() {
    if (readerAppearanceActivationPromise === guardedActivation) readerAppearanceActivationPromise = null
  })
  readerAppearanceActivationPromise = guardedActivation
  return guardedActivation
}

function openReaderPhoneAppearance(triggerElement) {
  return activateReaderAppearanceWorkbench(function(workbench, runtime) {
    return workbench.openPhoneAppearance(runtime, triggerElement)
  })
}

function openReaderProfileAppearance(triggerElement) {
  return activateReaderAppearanceWorkbench(function(workbench, runtime) {
    return workbench.openReaderProfile(runtime, triggerElement)
  })
}

function openReaderPerAppAppearance(type, triggerElement) {
  return activateReaderAppearanceWorkbench(function(workbench, runtime) {
    return workbench.openPerAppAppearance(runtime, type, triggerElement)
  })
}
...
document.addEventListener('click', async function(e) {
  ...
  if (ownerControl && ownerControl.closest('#tabCustom')) {
    ...
    if (ownerControl.dataset.readerPhoneControl === 'appearance') await openReaderPhoneAppearance(ownerControl)
    if (ownerControl.dataset.readerPhoneControl === 'profile') await openReaderProfileAppearance(ownerControl)
    return
  }
  ...
  while (el && el !== document.body) {
    if (el.classList && el.classList.contains('rd-app-icon')) {
      ...
      e.preventDefault()
      e.stopPropagation()
      await openReaderPerAppAppearance(type, el)
      return
    }
    el = el.parentElement
  }
...
// ---- Init ----
// Render-only surface. Normal reader pages never receive a parent-owned context.
export function createAuthorPhoneRenderSurface(context) {
  if (!authorPhoneRenderDocument || window.parent === window ||
      window.frameElement?.__tuuruAuthorPhoneRenderContext !== context) {
    throw new Error('作品渲染需要作者上下文')
  }
  context.assertCurrent()
  _work = JSON.parse(JSON.stringify(context.work))
  _work.readerPhValues = {}
  _readerPersistenceEnabled = false
  _editorPreviewMode = true
  resetReaderPhoneChoiceSession(_work)
  resetReaderPhoneFlowSession(_work)
  // Only established authored skin fields override defaults. Per-App reader
  // settings, custom CSS, reader fonts and reader identity are never inherited.
  var defaults = readerPhoneCustomDefaults()
  var skin = _work.phoneData.skin || {}
  authorPhoneRenderCustom = {}
  ;['wallpaper','wallpaperType','wallpaperImage','topBgImage','frameColor','borderRadius',
    'fontFamily','fontSize','showDynamicIsland','dynamicIslandStyle','showHomeIndicator',
    'showAppLabels','showIconShadow','iconBorderRadius','materialOpacity','timeColor'].forEach(function(key) {
    authorPhoneRenderCustom[key] = skin[key] === undefined ? defaults[key] : skin[key]
  })
  var pd = readerPhoneDataWithStoryState(_work.phoneData)
  var root = document.getElementById('app')
  root.innerHTML = buildPhoneHTML(pd, getPhoneCustom(), _work.watermark)
  var frame = root.querySelector('.phone-frame')
  if (!frame) throw new Error('作品小手机渲染失败')
  var navigation = {exportMode:true,exportFrame:frame}
  return {
    phoneData:pd,
    frame:frame,
    renderChat:function(chat,index,selections) {
      context.assertCurrent()
      openReaderChat(frame,_work,pd,chat,index,undefined,{
        exportMode:true,
        exportChoiceSelections:selections ? new Map(selections) : undefined,
      })
    },
    renderForum:function(postId,index) {
      context.assertCurrent()
      openReaderForumPost(frame,_work,pd,postId,index,navigation)
    },
    renderApp:function(type,contactIndex) {
      context.assertCurrent()
      openReaderApp(type,contactIndex,true,undefined,navigation)
    },
