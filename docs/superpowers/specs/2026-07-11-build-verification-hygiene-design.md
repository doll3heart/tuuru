...
1. 以 `os.tmpdir()` 为父目录调用 `mkdtemp()`，获得本次执行专属的绝对临时根目录。
2. 顺序构建编辑器和阅读器。
3. 每个构建继续加载仓库现有的真实 `configFile`，只以内联配置覆盖绝对 `build.outDir` 和 `emptyOutDir: true`。
4. 编辑器写入临时根目录下的 `editor/`，阅读器写入 `reader/`。
5. 在 `finally` 路径中只递归删除 `mkdtemp()` 返回的临时根目录。
