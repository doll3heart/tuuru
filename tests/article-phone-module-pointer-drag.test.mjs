...
test("article phone module arrangement is wired only through Pointer Events", () => {
  ...
  assert.match(editorSource, /class="pm-card-hamburger"[^>]*type="button"[^>]*aria-label="管理手机模块"/)
  assert.match(editorSource, /data-pm-act="move">移动到其他节点/)
  assert.match(editorSource, /data-pm-act="copy">复制到其他节点/)
  ...
  assert.match(css, /\.pm-menu-item\s*\{[^}]*min-height\s*:\s*44px/s)
  assert.match(css, /\.pm-menu-item:focus-visible\s*\{/)
})
