...
test("every authored chat card type routes back to its complete editor", () => {
  ...
  supports(/function showInlineStoryEditor\(type, title, placeholder, editingMessage\)/, "time cards")
  supports(/function showStoryEventEditor\(category, editingMessage\)/, "story and relationship events")
  ...
  supports(/\['location', 'contact-card', 'file', 'music', 'forward', 'schedule'\][\s\S]*addMsg\(msg\.type, msg\)/, "rich message context menu")
  supports(/msg\.type === 'system-event'[\s\S]*showStoryEventEditor\(eventCategory, msg\)/, "story event context menu")
})
