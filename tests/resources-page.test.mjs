...
test("critical tutorial button names are backed by the shipped interface", () => {
  ...
  assert.doesNotMatch(readerSource, /导出美化包/)
  assert.match(readerSource, /导入美化包/)
  assert.doesNotMatch(source, /导出美化包/)
...
test("the tutorial documents reader appearance package privacy in the existing file section", () => {
  ...
  assert.match(source, /不替换昵称、头像、读者 ID、阅读记录和作品内容/)
})
...
test("the tutorial explains phone image export, naming, pagination, masking, and external-image limits", () => {
  ...
  assert.match(source, /主角头像[^。]*NPC 与角色头像保持原样/)
  assert.match(source, /外链图片[^。]*跳过受影响的模块/)
...
test("the message tutorial explains the distilled composer and multi-select forwarding", () => {
  ...
  assert.match(source, /当前选项组还未选择[^。]*后面的剧情消息[^。]*隐藏/)
  assert.match(source, /剧情消息（包括系统消息）[^。]*暂时隐藏/)
  assert.match(source, /群聊使用【在前插入消息】时[^。]*选择读者、群成员或系统作为发送人/)
  assert.match(source, /重选前面的选项[^。]*清除[^。]*后续选择/)
  assert.match(source, /结束话题[^。]*角色后续[^。]*本轮[^。]*下一轮[^。]*重新判断/)
  assert.match(source, /跳过本轮剩余预设消息（包括系统消息）[^。]*发送间隔[^。]*逐条播放/)
  ...
  assert.match(source, /搜索[^。]*选项文字[^。]*会话或联系人[^。]*轮次[^。]*原消息[^。]*稳定 ID/)
