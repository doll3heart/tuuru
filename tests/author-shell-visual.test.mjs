...
test("mobile work cards align their action dividers while keeping distinct button grids", () => {
  ...
  assert.match(css, /\.work-card-actions\{display:grid;grid-template-columns:repeat\(3,minmax\(0,1fr\)\);grid-template-rows:44px/)
  assert.match(css, /\.work-card-actions-left\{display:contents\}/)
  assert.match(css, /\.work-card-phone \.work-card-actions\{grid-template-columns:repeat\(2,minmax\(0,1fr\)\)\}/)
  assert.doesNotMatch(css, /\.work-card-phone \.work-card-actions\{[^}]*grid-template-rows:32px/)
  assert.match(css, /\.work-card-actions \.btn\{width:100%;min-width:0;height:44px/)
  assert.match(css, /\.work-card-more-wrap\{width:100%;height:44px/)
