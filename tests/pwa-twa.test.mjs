...
test("the shared web manifest installs Tuuru and exposes author and reader shortcuts", async () => {
  ...
  assert.equal(icons.get("192x192")?.src, "/icons/tuuru-rabbit-v1-192.png")
  assert.equal(icons.get("512x512")?.src, "/icons/tuuru-rabbit-v1-512.png")
  assert.ok(manifest.icons.some(icon => icon.src === "/icons/tuuru-rabbit-v1-maskable-512.png" && /maskable/.test(icon.purpose)))
...
test("the service worker follows web deployments without forcing an editor reload", async () => {
  ...
  assert.match(worker, /CACHE_NAME\s*=\s*`\$\{CACHE_PREFIX\}v3`/)
  assert.match(worker, /tuuru-rabbit-v1-192\.png/)
...
test("the TWA contract remains bound to tuuru.chat and excludes signing secrets", async () => {
  ...
  assert.equal(config.iconUrl, "https://tuuru.chat/icons/tuuru-rabbit-v1-512.png")
  assert.equal(config.maskableIconUrl, "https://tuuru.chat/icons/tuuru-rabbit-v1-maskable-512.png")
  ...
  await access(new URL("public/icons/tuuru-rabbit-v1-192.png", root))
  await access(new URL("public/icons/tuuru-rabbit-v1-512.png", root))
  await access(new URL("public/icons/tuuru-rabbit-v1-maskable-512.png", root))
})
