...
test('native PNG references use matching Chromium text positioning without changing WebKit options', () => {
  assert.deepEqual(phoneExportBrowserOptions('chromium'), { headless:true, args:['--disable-lcd-text', '--disable-font-subpixel-positioning'] })
  ...
  assert.deepEqual(phoneExportBrowserOptions('chromium').args, ['--disable-lcd-text', '--disable-font-subpixel-positioning'])
})
