...
test("work cards consolidate export actions and keep delete inside More", () => {
  ...
  assert.equal(card.querySelector("[data-work-export]").textContent.trim(), "导出作品")
...
test("the work export sheet exposes only reliable local export actions", () => {
  window.openWorkExport("work-a")
  const overlay = document.querySelector(".work-export-overlay")
  assert.ok(overlay)
  assert.equal(overlay.querySelector(".modal-title").textContent.trim(), "导出作品")
  const shareNote = overlay.querySelector("[data-work-share-note]")
  assert.ok(shareNote)
  assert.match(shareNote.textContent, /主流平台.*压缩/)
  assert.match(shareNote.textContent, /作为.*文件.*发送/)
  assert.match(shareNote.textContent, /网盘.*下载链接/)
  assert.equal(shareNote.nextElementSibling, overlay.querySelector(".work-export-actions"))
  assert.equal(overlay.querySelector("[data-work-share]"), null)
  ...
  assert.equal(overlay.querySelectorAll(".work-export-actions button").length, 2)
  assert.equal(overlay.querySelector("[data-work-copy]"), null)
