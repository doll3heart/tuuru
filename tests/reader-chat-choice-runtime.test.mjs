...
async function openSeededChatWithProgress(t, work, progress, chatIndex = 0) {
  ...
  const readerModule = await import(`../reader/reader.js?reader-chat-choice-legacy=${Date.now()}-${Math.random()}`)
  document.querySelector('[data-tab="library"]').click()
  document.querySelector(".rd-recent-item").click()
  document.getElementById("rdStartBtn")?.click()
  document.querySelector('[data-app-type="messages"]').click()
...
test("detached export branches render independently without changing the live choice", async t => {
  const work = choiceWork()
  ...
  const readerModule = await openSeededChatWithProgress(t, work, {
    kind:"phone",
    flowIndex:0,
    phoneChoiceSelections:{ [owner.id]:"export-choice-a" },
  })
  const liveMessageArea = document.getElementById("chatMsgArea")
  await waitFor(() => liveMessageArea.textContent.includes("EXPORT_BRANCH_A_FOLLOW_UP"))
  assert.doesNotMatch(liveMessageArea.textContent, /EXPORT_BRANCH_B_/)
  const savedBeforeExport = localStorage.getItem("moirain_readerLibrary")

  document.getElementById("chatBack").click()
  const liveFrame = document.querySelector(".phone-frame")
  const exportFrame = liveFrame.cloneNode(true)
  exportFrame.querySelectorAll("[id]").forEach(element => element.removeAttribute("id"))
  document.body.appendChild(exportFrame)
  t.after(() => exportFrame.remove())
  ...
  exportFrame.remove()
  document.querySelector('.rd-chat-card[data-chat-index="0"]').click()
  const reopenedLiveMessageArea = document.getElementById("chatMsgArea")
  await waitFor(() => reopenedLiveMessageArea.textContent.includes("EXPORT_BRANCH_A_FOLLOW_UP"))
  assert.doesNotMatch(reopenedLiveMessageArea.textContent, /EXPORT_BRANCH_B_/)
  assert.equal(localStorage.getItem("moirain_readerLibrary"), savedBeforeExport)
})
...
test("all-branch planner exposes only masked branch labels in its progress item labels", async t => {
  ...
  document.getElementById("chatBack").click()
  const exportFrame = document.querySelector(".phone-frame").cloneNode(true)
  exportFrame.querySelectorAll("[id]").forEach(element => element.removeAttribute("id"))
  document.body.appendChild(exportFrame)
  t.after(() => exportFrame.remove())
...
test("phone export planning enforces the aggregate 64-branch limit before rendering", async t => {
  ...
  const exactLimitError = new Error(
    "可导出的消息选项分支超过 64 条，请减少选项后重试，或改用“当前阅读分支”。",
  )
