...
function buildInteractiveExperienceEditor(work) {
  ...
  return '<main class="mini-game-studio app-main">'
    ...
    + '<div class="mini-game-layout"><div class="mini-game-primary-column">'
    ...
    + '<button type="button" class="btn btn-primary" data-a="mini-scene-edit" data-w="' + escAttr(work.id) + '">' + (scene ? '编辑画面与互动' : '创建第一个互动场景') + '</button></section></div>'
    + '<div class="mini-game-settings-column">'
    ...
    + '<p class="mini-game-help">开场前收集姓名等答案，并替换画面中的提示、说话人、台词或选项。</p>'
    ...
    + '<p class="mini-game-help">默认贯穿全篇；画面特殊 BGM 结束后自动恢复。</p>'
...
'<p><b>违禁词：</b>单项词只限制当前占位符，全局词会叠加到本作品全部占位符。包含匹配会拦截含有该词的长句；完全匹配只拦截去除首尾空格后整段相同的答案，两者都忽略大小写。点击【整理全部词库】可拆词、清理空项并去重。</p>' +
'<p><b>作者预设：</b>套用后会新增可独立修改的占位符副本；预设中的全局违禁词会与当前作品词库合并去重，不会覆盖原词。</p>' +
