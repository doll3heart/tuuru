...
const TUTORIAL_FEATURE_SECTIONS = [
  ...
  },
  {
    id:"phone", title:"小手机", features:[
      ...
      { title:"文字消息出现方式", what:"决定一条文字气泡是逐字显示，还是整条直接显示。", where:"普通文字消息：手机或 iPad 长按，电脑右键，再点【出现方式】；回复选项里的角色后续则展开该条的【发送设置】。", use:"选择【逐字出现】或【整条出现】并保存。旧作品和没有单独设置的消息默认逐字出现。", effect:"设置只改变这一条文字的呈现动画，不改变发送间隔、输入状态或剧情顺序；文字、图片、系统消息和特殊卡片始终按作者添加的原顺序逐条播放。" },
      ...
      { title:"单条消息发送间隔", what:"精确控制一个气泡要在上一条发出多久后才出现。", where:"手机或 iPad 长按任意消息；电脑右键任意消息，再点【发送间隔】。回复选项中的角色后续消息则展开该条的【发送设置】。", use:"在【发送前等待（秒）】填写 0 到 60 秒，可精确到 0.1 秒；留空沿用默认 0.8 秒。这里填写的是当前消息发送前的等待时间。", effect:"普通剧情消息和回复选项后的角色气泡都会逐条按各自间隔出现。发送间隔只决定气泡何时出现，不会自动显示“正在输入”；需要输入状态时另用【回复节奏】。" },
...
const TUTORIAL_FAQ_SECTIONS = {
  ...
  phone:[
    ...
    { question:"怎样单独设置某条消息隔多久发出？", answer:"手机或 iPad 长按任意消息，电脑右键任意消息，点【发送间隔】，填写 0 到 60 秒。回复选项中的角色后续要先展开该条的【发送设置】，再填写【发送前等待（秒）】；留空使用默认 0.8 秒。" },
