  ...
  return `<div class="resource-panel" data-resource-panel="tutorial">
    ...
    ${tutorialGuide({
      ...
      steps:[
        ...
        { title:"插入普通互动", body:"<p>把光标放在希望提问的位置，点正文工具栏的「普通互动」。一段正文可以插入多组；分别填写按钮上的「选项文本」和点击后出现的「选择后内容」。普通互动卡片可以再次编辑、删除或移动到新的光标位置。</p>" },
        { title:"连接剧情分支", body:"<p>点「剧情分支」，填写选项文字并选择目标节点。承担跳转的剧情分支固定显示在节点正文末尾，不会混进中间的普通互动卡片。</p>" },
      ...
      faq:[
        ...
        { title:"让两个选择走向不同结局", body:"<p><strong>入口：</strong>节点正文工具栏 →「剧情分支」。添加两条剧情选项，并分别选择目标节点；这一组会固定在节点末尾。</p>" },
...
const TUTORIAL_FEATURE_SECTIONS = [
  ...
  },
  {
    id:"article", title:"互动文章", features:[
      ...
      { title:"剧情分支", what:"让读者的选择跳转到另一条剧情路线，并记录本次选择。", where:"节点正文工具栏 → 剧情分支。", use:"填写选项文本，再指定目标章节和目标节点。剧情分支始终固定在节点正文末尾，不能移动到正文中段；被它直接指向的普通节点会自动属于对应路线，不需要另外设置显示条件。", effect:"读者点击后只显示所选目标路线，不会同时显示同组选项指向的并行节点；每个选项的稳定 ID 会用于后续显示条件，修改选项文字不会改变已有条件的关联。" },
      { title:"普通互动", what:"在正文任意位置提供不跳转的反应选择；同一节点可以容纳多组。", where:"把光标放进正文 → 正文工具栏 → 普通互动。", use:"每组至少添加两个选项，分别填写按钮上显示的“选项文本”和点击后出现的“选择后内容”。选择后内容可以分行，每一行都会按独立正文段落显示。点正文里的互动卡片可继续编辑、删除或进入移动状态，再点击正文中的新位置完成放置；若位置丢失，可从待放置提示恢复到光标处。", effect:"各组分别记录读者选择，互不覆盖，也不会改变当前剧情路线；反馈跟随正文的字体、字号、行距、字距、段落间距、首行缩进和安全自定义样式。普通互动与节点末尾的剧情分支可以同时存在。" },
      ...
      { title:"显示条件（或 / 且）", what:"用读者曾经选择过的普通互动或剧情分支决定隐藏节点是否显示。", where:"选中隐藏节点 → 编辑器顶部 → 显示条件。", use:"搜索选项文本、普通互动组名、节点、章节或稳定 ID并加入条件。结果会标明“普通互动”或“剧情分支”。同一组中的选项使用“或”，不同条件组之间使用“且”；例如“同意或沉默”且“曾经拒绝”。需要同时满足更多条件时，点击“添加附加条件（且）”。", effect:"条件根据稳定 ID 读取选择记录；同一节点里的多组普通互动可以分别作为条件。选项改名后仍能继续关联，选项被删除或引用无法确定时会显示“条件已失效”，修复或移除失效条件后才能保存。" },
