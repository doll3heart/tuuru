...
const TUTORIAL_FEATURE_SECTIONS = [
  ...
  },
  {
    id:"interactive", title:"互动图片与 Mini文游", features:[
      { title:"Mini文游适合做什么", what:"在一个互动场景里安排多个固定顺序画面，制作以图片为主、一次读完的短篇互动体验。", where:"创作端页面顶部【新建】里的【Mini文游】。", use:"适合小段故事、氛围互动、角色特典或绘画展示。每个画面可以放背景、立绘、叠加图层、热区、提示、对话、动作帧和音乐。", effect:"Mini文游不是完整文游引擎；不提供自由剧情分支、多个结局、变量、背包、数值养成、战斗、自定义脚本或小手机 App。需要复杂路线时，请创建互动文章并把互动图片页放进正文路线。" },
      ...
      { title:"安排画面顺序与完成条件", what:"决定读者按什么顺序探索画面，以及什么时候可以继续。", where:"画面编辑器左侧【画面】列表和列表下方的操作区。", use:"点【＋ 添加画面】增加内容。选中画面后用【上移】或【下移】调整顺序，用【删除】移除误建画面。有热区的画面必须探索完全部热区，才能点击主对话框继续；没有热区的画面可以直接继续。", effect:"列表第一项就是读者起点，画面按列表顺序推进；最后一个画面完成后进入作品完成页，读者可以重新开始或返回书架。热区只产生反馈，不决定下一画面。" },
      ...
      { title:"管理多个画面", what:"在 Mini文游或文章互动页中安排依次出现的画面。", where:"打开画面编辑器，看左侧【画面】列表。", use:"点【＋ 添加画面】建立画面 2、画面 3，点画面名称切换，用【上移】【下移】调整顺序。读者探索完当前画面的全部互动点后，点击对话框进入下一画面；没有互动点时可直接点击。", effect:"画面严格按左侧顺序推进，不需要在每个热区中重复指定下一张图，也不会在画面内部产生剧情分支。Mini文游在最后一个画面后结束；文章互动页会继续到指定节点。" },
      ...
      { title:"说话人、台词与文章占位符", what:"为每个画面和热区分别安排角色名与文字反馈。", where:"画面设置中的「说话人、初始台词」；热区设置中的「触发时说话人、触发时台词」。", use:"画面台词负责进入画面时的内容；热区台词负责触发后的反应。互动文章里的互动页还可以写入当前文章已有的占位符标记；Mini文游专用页没有占位符创建或管理入口。", effect:"互动文章读者端会替换触摸提示、说话人、初始台词和热区台词中的占位符；不会替换图片链接、视频链接或内部 ID。" },
  ...
  },
  {
    id:"phone", title:"小手机", features:[
      ...
      { title:"回复节奏", what:"分别控制每个读者选项之后，角色接话出现得有多快。", where:"手机或 iPad 长按拥有回复选项的消息；电脑右键该消息，再点【设置回复节奏】。也可在回复选项编辑器中直接选择。", use:"逐个选项选择【直接出现】【很快回复】【正在输入后回复】或【稍后回复】，再保存。", effect:"非即时节奏会先显示输入状态，再按所选等待时间展示角色后续气泡；不同选项可以使用不同速度。" },
  ...
  },
  {
    id:"placeholders", title:"占位符", features:[
      ...
      { title:"单项违禁词", what:"只限制一个占位符的部分输入。", where:"进入【占位符管理】，找到目标占位符卡片中默认折叠的【违禁词 · N 个】。", use:"点击标题展开，输入词语；可用换行、逗号、顿号、分号、斜杠或竖线分隔，最后保存。标题旁词数会随输入更新。", effect:"该占位符命中词语时会提示修改；折叠后只保留词数，不占用整张卡片高度。" },
      { title:"全局违禁词", what:"让同一作品的所有占位符共用一份限制词。", where:"进入【占位符管理】，找到顶部默认折叠的【全局违禁词 · N 个】。", use:"点击标题展开，输入词语后点【整理全部词库】。小手机还要点设置页底部【保存】。", effect:"任一占位符命中词语时都会提示修改；每张占位符卡片可以展开查看继承的全局词。" },
...
const TUTORIAL_FAQ_SECTIONS = {
  ...
  interactive:[
    { question:"Mini文游和互动图片的画面能力一样吗？", answer:"两者共用同一套画面编辑与播放能力，包括固定画布、背景、立绘、叠加图层、热区、提示、对话、动作帧和画面特殊 BGM；同样的素材组合在播放时需要相近的解码内存。Mini文游是可单独导入书架的短篇外壳，只有一个互动场景，可设置作品默认 BGM，最后进入完成页；互动图片位于互动文章路线中，一篇文章可以放置多个，结束后进入作者指定的普通节点，并可使用文章占位符。使用 HTTPS 链接只会减小导出包和本地存储，不会降低读者播放时的图片、GIF 或视频解码内存。" },
    ...
    { question:"Mini文游是不是完整文游引擎？", answer:"不是。Mini文游定位为以图片、热区、对话、动作帧和 BGM 组成的短篇固定顺序互动体验，不提供自由剧情分支、背包、数值养成、战斗或自定义脚本。需要多路线长剧情时，请使用互动文章，并在需要的位置插入互动图片页。" },
    { question:"Mini文游可以在画面中分支吗？", answer:"不能。画面按左侧列表的固定顺序推进，同一画面的热区只负责探索和反馈，不会跳到不同画面。需要分支时，把不同互动页放进互动文章的不同剧情路线。" },
    ...
    { question:"【互动文章】占位符可以写在哪些互动文字里？", answer:"文章互动页的触摸提示、画面说话人、初始台词、热区说话人和热区台词都支持当前文章已有的占位符。图片或视频链接、内部 ID 不会被替换。Mini文游专用页没有占位符创建或管理入口。" },
  ...
  phone:[
    ...
    { question:"怎样让角色先显示正在输入再回复？", answer:"手机或 iPad 长按拥有回复选项的消息，电脑右键该消息，点“设置回复节奏”。为每个选项选择“很快回复”“正在输入后回复”或“稍后回复”；不需要等待时选“直接出现”。" },
