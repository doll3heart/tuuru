...
const TUTORIAL_FEATURE_SECTIONS = [
  ...
  },
  {
    id:"phone", title:"小手机", features:[
      ...
      { title:"消息编辑菜单", what:"修改消息，并安排撤回或发送失败等聊天过程。", where:"手机或 iPad 长按消息；电脑右键消息。", use:"选择编辑、引用、多选、撤回、发送失败、在前插入时间、在前插入消息或添加选项。群聊使用【在前插入消息】时，可先选择读者、群成员或系统作为发送人；撤回后可从同一菜单取消撤回。", effect:"发送失败时，读者会先看到消息发出，再看到它迅速消失；撤回时，原消息会替换为可点开查看原文的系统提示。" },
      ...
      { title:"消息回复选项", what:"在一条消息后加入一个或多个读者选项组，并安排角色接话。", where:"手机或 iPad 长按目标消息；电脑右键目标消息。", use:"在消息菜单点【添加选项】。每组填写【选项文字】和【发送内容】，需要更多分支时点【添加新选项组】。展开【图片与更多设置】可添加回复图片、沉默、结束话题、点赞数和默认回复节奏；在【角色后续】点【添加消息】，逐条选择发送者。展开每条后续的【发送设置】，可选择【正常发送】【发送失败】或【发送后撤回】，并设置单条节奏和发送间隔。", effect:"没有图片时，读者选择后发送【发送内容】；设置图片后会显示图片选项，并发送同一张图片，选项文字只作说明。每条后续都会按所选角色的身份显示。沉默不会生成读者气泡，但仍会记录选择并触发后续；结束话题会隐藏本轮后面的普通消息。多组选项会按消息顺序出现；当前选项组还未选择或角色后续仍在播放时，后面的剧情消息（包括系统消息）会暂时隐藏，再按作者顺序逐条出现；位于当前选项前面的系统消息会保持显示。重选前面的选项会清除它之后的角色接话和后续选择。" },
      { title:"小手机剧情显示条件", what:"让一条消息或论坛帖子在读者作出指定选择后才出现。", where:"消息：长按或右键气泡，点【剧情显示条件】；论坛：打开帖子，点【编辑帖子】。", use:"在【剧情显示条件】中选择已有消息回复，例如“选过『拒绝』后显示”。留在【始终显示】则不参与判定。", effect:"条件消息会在对应选择后进入会话；条件帖子此前不会出现在论坛列表，解锁后看起来像刚刚更新。选择会保存在当前作品的本地阅读档案中。" },
...
const TUTORIAL_FAQ_SECTIONS = {
  ...
  phone:[
    ...
    { question:"角色后续回复怎样分成多个气泡？", answer:"打开消息的【添加选项】，在【角色后续】中每点一次【添加消息】就新增一个气泡；逐条选择发送人并填写内容。消息状态、单条节奏和发送间隔在该条的【发送设置】中。换行不会自动拆成多个气泡。" },
