...
const TUTORIAL_FEATURE_SECTIONS = [
  ...
  },
  {
    id:"phone", title:"小手机", features:[
      ...
      { title:"消息回复选项", what:"在一条消息后加入一个或多个读者选项组，并安排角色接话。", where:"手机或 iPad 长按目标消息；电脑右键目标消息。", use:"在消息菜单点【添加选项】。每组填写【选项文字】和【发送内容】，需要更多分支时点【添加新选项组】。展开【图片与更多设置】可添加回复图片、沉默、结束话题、点赞数和默认回复节奏；在【角色后续】点【添加消息】，逐条选择发送者。展开每条后续的【发送设置】，可选择【正常发送】【发送失败】或【发送后撤回】，并设置单条节奏和发送间隔。", effect:"没有图片时，读者选择后发送【发送内容】；设置图片后会显示图片选项，并发送同一张图片，选项文字只作说明。每条后续都会按所选角色的身份显示。沉默不会生成读者气泡，但仍会记录选择并触发后续；结束话题会隐藏本轮后面的普通消息。多组选项会按消息顺序出现；当前选项组还未选择时，后面的剧情消息会暂时隐藏，同一段里的系统消息仍会正常显示。重选前面的选项会清除它之后的角色接话和后续选择。" },
