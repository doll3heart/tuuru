...
const TUTORIAL_FEATURE_SECTIONS = [
  ...
  },
  {
    id:"phone", title:"小手机", features:[
      ...
      { title:"单条消息发送间隔", what:"精确控制一个气泡要在上一条发出多久后才出现。", where:"手机或 iPad 长按任意消息；电脑右键任意消息，再点【发送间隔】。回复选项的每条【角色后续消息】里也有同名输入框。", use:"填写 0 到 60 秒，可精确到 0.1 秒；留空沿用默认 0.8 秒。这里填写的是当前消息发送前的等待时间。", effect:"普通剧情消息和回复选项后的角色气泡都会逐条按各自间隔出现。发送间隔只决定气泡何时出现，不会自动显示“正在输入”；需要输入状态时另用【回复节奏】。" },
      ...
      { title:"群聊 @ 全体成员", what:"在群聊消息中插入面向所有成员的提及。", where:"群聊底部消息输入框，以及群聊回复选项的读者回复和角色后续消息输入框。", use:"输入 @，从候选列表选择【全体成员】；也可以继续选择单个联系人、读者称呼或作品占位符。", effect:"保存后按 @全体成员 显示。这个候选项只在群聊出现，单聊和论坛不会混入。" },
...
const TUTORIAL_FAQ_SECTIONS = {
  ...
  phone:[
    ...
    { question:"角色后续回复怎样分成多个气泡？", answer:"打开消息的【添加选项】，在【角色后续消息】中每点一次【添加】就新增一个气泡；逐条选择发送人、消息状态与单条节奏，再填写内容。换行不会自动拆成多个气泡。" },
    { question:"怎样让角色先显示正在输入再回复？", answer:"手机或 iPad 长按拥有回复选项的消息，电脑右键该消息，点“设置回复节奏”。可以先设置整组选项的默认速度，再打开回复选项编辑器，让某条角色消息继承本组选项或单独选择“很快回复”“正在输入后回复”“稍后回复”；不需要输入状态时选“直接出现”。【发送间隔】是另一项设置，只负责当前气泡在上一条之后等待几秒。" },
    { question:"怎样单独设置某条消息隔多久发出？", answer:"手机或 iPad 长按任意消息，电脑右键任意消息，点【发送间隔】，填写 0 到 60 秒；回复选项中的每条角色后续消息也能单独填写。留空使用默认 0.8 秒。这个时间从上一条消息出现后开始计算。" },
