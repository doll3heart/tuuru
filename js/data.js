...
export function deleteNode(wid,nid){const db=rd();const w=db.works.find(x=>x.id===wid);if(!w)return;const removed=w.nodes.find(x=>x.id===nid);w.nodes=w.nodes.filter(x=>x.id!==nid);w.nodes.forEach(x=>{x.choices=x.choices.filter(c=>c.targetId!==nid);x.interactionGroups=(x.interactionGroups||[]).map(g=>g?.kind==="random-game"?{...g,choices:(g.choices||[]).map(c=>c?.targetId===nid?{...c,targetId:""}:c)}:g)});if(removed&&Array.isArray(w.interactiveScenes)){w.interactiveScenes=w.interactiveScenes.filter(scene=>scene.id!==removed.interactiveSceneId&&scene.nodeId!==nid)}ensureArticleStartNode(w);w.updatedAt=Date.now();wr(db)}
...
export function addForumPost(wid,data){const db=rd();const w=db.works.find(x=>x.id===wid);if(!w||!w.phoneData)return null;const p={id:uid(),platform:data.platform||PLATFORM.X,contactId:data.contactId||"",content:data.content||"",images:data.images||[],time:data.time||"",pinned:false,featured:false,likes:[],reposts:[],comments:[]};w.phoneData.forumPosts.push(p);w.updatedAt=Date.now();wr(db);return p}
